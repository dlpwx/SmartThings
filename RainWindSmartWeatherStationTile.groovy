...
metadata {
    ...
    valueTile("windDir", "device.windDir", decoration: "flat") {
		...
		main(["temperature", "weatherIcon", "rain"])
		details(["temperature", "weatherIcon","rain","feelsLike","wind","windDir","weather", "city","percentPrecip", "refresh","alert", "humidity","rise","set","light"])}
}
...
def poll() {
	...
	if (obs) {
		...
		send(name: "rain", value: obs.precip_today_in as String, unit: "in") // as String because of bug in determining state change of 0 numbers
    ...
    send(name: "windDir", value: obs.wind_dir)
		...
		send(name: "wind", value: obs.wind_gust_mph as String, unit: "MPH") // as String because of bug in determining state change of 0 numbers
